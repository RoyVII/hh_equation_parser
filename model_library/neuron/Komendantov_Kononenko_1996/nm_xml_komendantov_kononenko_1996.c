	...
	while (cur != NULL) {
		if (xmlStrcmp(cur->name, (const xmlChar *) "vars") == 0) {
			...
			while (child != NULL) {
				...
				if (xmlStrcmp(child->name, (const xmlChar *) "m_b") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_M_B], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "h_b") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_H_B], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "m") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_M], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "h") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_H], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "n") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_N], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "m_ca") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_M_CA], (const xmlChar*) VALUE);
				if (xmlStrcmp(child->name, (const xmlChar *) "Ca") == 0) ret = parse_double(doc, child, &args->vars[NM_KOMENDANTOV_KONONENKO_1996_CA], (const xmlChar*) VALUE);
			...
			while (child != NULL) {
				...
				if (xmlStrcmp(child->name, (const xmlChar *) "syn") == 0) ret = parse_double(doc, child, &args->params[NM_KOMENDANTOV_KONONENKO_1996_SYN], (const xmlChar*) VALUE);
